...
if (new Plugins('track')) {
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        function(position) {
          ...
          //   })
          // });
          zsld.VECTORS.add('DEVICE_TRACKING').addFeatures([accuracyFeature, positionFeature], { clear: true, activate: true, append: false, overwrite: true });
        },
        function(position) {
  ...
  Plugins.track.initializeForm = function() {
    ...
    if (REGISTRATIONDIALOG || (QUERY_STRING.tic && REGISTERED_DEVICE < 1) || (REGISTERED_DEVICE < 1 && REGISTRATION_PROHIBITED != "1")) {
